...
├── .env                      # Bí mật (không commit) — xem `.env.example` (HF + Git)
├── .env.example              # Mẫu: chỉ `HF_TOKEN` / `GIT_TOKEN`
...
2. Sao chép `.env.example` → `.env` (cùng thư mục với `configs/`), điền **chỉ** token nếu cần:

   - `HF_TOKEN` — push / tải private trên Hugging Face (tùy chọn: `HUGGINGFACE_HUB_TOKEN` nếu tool của bạn chỉ đọc tên đó).
   - `GIT_TOKEN` — thao tác GitHub API / clone private (nếu pipeline của bạn dùng).

   Siêu tham số train, đường dẫn dữ liệu, Hub: chỉnh **`configs/logic_model.yaml`** và **`configs/fol_model.yaml`** (file theo Git).
...
3. Thứ tự cell:
   ...
   - **Drive** chỉ khi `data.data_source: drive` trong `configs/logic_model.yaml` (gọi `download_and_extract_from_drive` trong cell).
   - **Train:** `run_training(cfg)` rồi `run_test_eval(...)` (xem cell trong `logic_model_pipeline_official.ipynb`).
   - **Push Hub:** `push_merged_lora(cfg, token)` khi `hub.push_to_hub: true` trong `configs/logic_model.yaml`.
...
5. Siêu tham số: chỉnh **`configs/logic_model.yaml`** (mục `eval` cho `log_test_each_epoch`, `experiment_inference_sample_n`, …). Module `models/logic_model/hyperparameters_tuning.py` có thể đọc YAML (cần `pyyaml`).

6. **Log phiên bản / Hub:** sau train, `notebooks/output/.../experiment_log.json` gom siêu tham số (cấu trúc gần `configs/logic_model.yaml`), toàn bộ `trainer_log_history`, và bảng **theo từng epoch**: loss train (log gần nhất), `eval_loss` / `eval_accuracy` trên **dev**, `test_accuracy` trên **test** (tắt bằng `eval.log_test_each_epoch: false` trong YAML, hoặc export `LOGIC_LOG_TEST_EACH_EPOCH=false` trên CI). Sau `run_test_eval`, file được bổ sung `test_accuracy` + mẫu inference (số mẫu: `eval.experiment_inference_sample_n` trong YAML). Khi `push_merged_lora`, các file này được đẩy lên Hub trong `experiment_artifacts/`.
...
2. Mở `notebooks/fol_model_pipeline_official.ipynb` và chạy **theo thứ tự phase**: bootstrap → cấu hình (VRAM: `training.load_in_8bit`, `training.gpu_profile`, `training.use_unsloth` trong **`configs/fol_model.yaml`**; eval đủ split: `eval.eval_fol_max_samples: null`) → build dataset + xem prompt → **baseline** (`fol_preflight`) → **SFT** → **push** (benchmark + latency) → **Hub reload** (N mẫu test ngẫu nhiên theo `eval.hub_reload_random_test_n`).
3. **CLI (terminal, tránh timeout kernel notebook):** từ gốc repo:

   ```bash
   cd Logic_Based_Educational_Queries_Project
   export PYTHONPATH=src   # Makefile train-fol đã set; giữ nếu gọi python -m trực tiếp
   make train-fol
   # tương đương: PYTHONPATH=src python -m models.fol_model.train
   ```

   Sau train, nếu `push_to_hub: true` trong `configs/fol_model.yaml` **và** có `HF_TOKEN` hoặc `HUGGINGFACE_HUB_TOKEN` (thường trong `.env`), pipeline sẽ **merge LoRA + push** như notebook; nếu `eval.hub_reload_after_push: true` (mặc định), sau push sẽ **tải lại merged từ Hub**, chạy greedy trên **N mẫu test ngẫu nhiên** (`hub_reload_random_test_n`), ghi `fol_hub_reload_eval.json`, in log trên terminal, upload artifact và cập nhật README Hub (xem trước ~2 mẫu). Tắt push: `push_to_hub: false` trong YAML hoặc `export FOL_SKIP_HUB_PUSH=1` một lần trên shell. Tắt reload: `hub_reload_after_push: false` trong YAML.

   **tmux** (tách session, SSH đứt vẫn chạy):

   ```bash
   tmux new -s fol
   make train-fol
   # Ctrl+B rồi D = detach; tmux attach -t fol = vào lại
   ```

   **nohup** (log ra file):

   ```bash
   nohup make train-fol > fol_train.log 2>&1 &
   tail -f fol_train.log
   ```

   Cùng bước Hub reload + N mẫu test đã nằm trong `python -m models.fol_model.train` sau push (xem `eval` trong `configs/fol_model.yaml`).

   **Log & artifact (CLI = cùng quy ước notebook):** mặc định ghi vào `notebooks/output/pipeline_fol_sft/` (xem `prepare_fol_training_paths` trong `src/data/fol_dataset.py`) — gồm `experiment_log.json`, `train_metrics.json`, `fol_eval_metrics.json`, `fol_test_benchmark.json`, `fol_inference_latency.json` (nếu bật), checkpoint `final_lora/`, sau push thêm `merged_for_hub/` và `fol_hub_reload_eval.json`. Baseline trước FT (`fol_preflight_baseline.json`) chỉ có nếu bạn đã chạy bước preflight ít nhất một lần (notebook hoặc script tương đương); thiếu file này thì `fol_test_benchmark` vẫn ghi nhưng phần “trước FT” để trống.
...
| `make train-fol` | `python -m models.fol_model.train` — train + eval FOL; nếu `push_to_hub` + token thì **merge + push Hub** (trừ khi `FOL_SKIP_HUB_PUSH=1`), rồi tuỳ cấu hình **Hub reload + eval + mẫu test ngẫu nhiên** (`fol_hub_reload_eval.json`). Chạy lâu: dùng **tmux** / **nohup** (xem README mục FOL). |
