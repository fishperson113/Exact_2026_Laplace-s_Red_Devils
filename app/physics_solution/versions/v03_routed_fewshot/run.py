"""v03 runner — routed few-shot (2-pass pipeline).

Pass 1: classify each question into (domain, answer_type) using the model.
Pass 2: solve with domain-specific system prompt + matched few-shot examples.

Uses a custom loop instead of run_solver() because each row needs a
different prompt template based on its classification result.
"""
...
from app.physics_solution.versions.v03_routed_fewshot.prompts import (
    build_routed_template,
    CLASSIFY_SYSTEM,
    CLASSIFY_EXAMPLES,
)
from app.physics_solution.versions.v03_routed_fewshot import (
...
def _load_examples() -> list[dict]:
    if not EXAMPLES_PATH.exists():
        raise FileNotFoundError(
            ...
            "`python -m app.physics_solution.versions.v03_routed_fewshot.select_fewshot --k 2` first."
        )
...
def _select_examples(
    ...
    # Fallback 2: same answer_type, any domain
...
def run(args) -> dict:
    ...
    # Routing accuracy check (compare predicted domain vs ID prefix)
    correct_routes = sum(
